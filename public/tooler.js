...
function compute(a, b, t) {
    ...
    var firstBound = ClipperLib.JS.BoundsOfPath(subj[0], 1);
    ...
    if(solution.length > 1){
        ...
        for(var i = 0; i < solution.length; i++){
            var bound = ClipperLib.JS.BoundsOfPath(solution[i], 1);
            if(isInsideBound(firstBound, bound)){
                console.log("主框", i);
                id = i;
            }
        }
        var aim = solution.splice(id, 1)[0];
        solution.unshift(aim);
    }

    return solution;
}

function isInsideBound(a, b){
